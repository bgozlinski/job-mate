...
async def test_the_score_is_the_share_of_keywords_the_resume_covers(
    ...
    async with session_factory() as session:
        ...
        full = await match_resume(session, JOB_POST, job_post, writer, prompts)
        none = await match_resume(session, "nothing at all", job_post, writer, prompts)
        partial = await match_resume(session, RESUME, job_post, writer, prompts)
...
async def test_the_prompt_carries_the_posting_being_matched_and_no_other(
    ...
    async with session_factory() as session:
        ...
        result = await match_resume(session, RESUME, job_post, writer, prompts)
...
async def test_the_prompt_says_what_notes_is_for(
    ...
    async with session_factory() as session:
        ...
        await match_resume(session, RESUME, job_post, writer, prompts)

    assert "notes" in writer.prompts[0]


async def test_the_posting_behind_the_answer_is_recorded_for_audit(
    session_factory, model, cache, writer, prompts
):
    """The text the model saw is exactly these chunks, joined."""
    async with session_factory() as session:
        job_post = await store(session, model, cache, JOB_POST)
        await store(session, model, cache, OTHER_POST)
...
async def test_a_knowledge_base_of_one_posting_still_produces_a_match(
    session_factory, model, cache, writer, prompts
):
    async with session_factory() as session:
        job_post = await store(session, model, cache, JOB_POST)
