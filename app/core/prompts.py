...
afterwards, so editing a prompt is not the same act as shipping it.
"""

CACHE_TTL_SECONDS = 300
"""
How long a fetched prompt is served from memory. Long, because a prompt changes a few
times a day at most, and every miss is a blocking request on the path of an LLM call. An
expired entry is refreshed in a background thread and the stale text is served
meanwhile, so this is the only fetch anybody waits for.
"""

FETCH_TIMEOUT_SECONDS = 3
FETCH_RETRIES = 1
"""
Both deliberately short: a prompt server that is slow must not become the latency of the
application. What is waited for here is an improvement on a text that is already in the
process (NFR-2).
"""

PLACEHOLDER = re.compile(r"\{\{\s*(\w+)\s*\}\}")
"""
What a variable looks like. Whitespace inside the braces is allowed because the Langfuse
editor puts it there.
"""
...
"""
"""
One side of the comparison, and the reason the spelling rule exists twice: what a
posting demands and what a resume evidences are read by two separate calls, so a
normalisation that only one of them performs is worse than none. British spelling is
picked for being one choice, not for being right.
"""
...
"""
"""
The other half of the comparison. The instruction to expand an abbreviation is what this
whole call is for: the resume says k8s and the posting says kubernetes, the resume says
cat c+e and the posting says category c+e licence, and no rule about plurals will ever
...
"""
"""
The sections are part of the prompt rather than of the code: what the model is shown, in
what order, under which heading, is the thing being tuned. The code only decides what
goes into each, and says "none" where there is nothing.
"""
...
"""
"""
What the deterministic rule cannot do (W-1). It compares whole terms, so postgresql
never answers sql and no rule about plurals will make it; a model reads the sentence
instead.
"""
...
would store them under, so moving a text out of here changes where it is read from and
nothing else.
"""
