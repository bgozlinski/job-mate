...
export function Match(): ReactElement {
  ...
  return (
    <>
      <h2>Match a resume against a posting</h2>

      {noResumes ? (
        <p>
          No resumes yet. <Link to="/resumes">Add one</Link> first.
        </p>
      ) : null}
      ...
      {noDocuments ? (
        <p>
          ...
          <Link to="/documents">Add a job posting</Link> first.
        </p>
      ) : null}

      <form aria-label="Match a resume" onSubmit={onSubmit}>
        ...
        <label htmlFor="match-document">Job posting</label>
        <select
          id="match-document"
          required
          value={documentId}
